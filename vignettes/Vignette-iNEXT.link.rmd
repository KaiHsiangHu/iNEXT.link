...
The five classes of unevenness measures developed in Chao and Ricotta (2019) are used to measure network specialization. Each unevenness measure is parameterized by an order q > 0; for q = 0, interaction frequencies are disregarded, so it is not meaningful to evaluate (un)evenness among frequencies. For each class, one can construct a continuous profile that depicts the measure as a function of diversity order q > 0. All five classes are functions of taxonomic network diversity and interaction richness. Because interaction richness often cannot be accurately estimated for incomplete sampling data, unevenness/specialization profiles across networks can only be compared at a standardized coverage level, up to a maximum coverage value of C_max, defined in Step 3.

The following step was not included in the proposed procedure in Chiu et al. (2023), but is featured in this R package.
...
Two functions for specialization analysis with graphical displays: 

- **Spec.link** computes standardized specialization estimation under specified sample coverage(or observed) with order q.

- **ggSpec.link** visualizes the output from the function `Spec.link`
...
To conduct phylogenetic network diversity analysis, the phylogenetic tree (in Newick format) spanned by species observed in all datasets must be stored in a data file. For example, the phylogenetic tree for all observed species (including species in "Closed" and "Open" fragments) is stored in a data file named `"beetles_col_tree"` for demonstration purpose. A partial list of the tip labels and node labels are shown below. 
...
To compute functional network diversity, the species-pairwise distance matrix (Gower distance computed from species traits) for species observed in all datasets must be stored in a matrix/data.frame format. Typically, the distance between any two species is computed from species traits using the Gower distance. In our demo data, the distance matrix for all species (including species in "Closed", and "Open" fragments) is stored in a data file named `"beetles_col_distM"` for demonstration purpose. Here we only show the first three rows and three columns of the distance matrix.
...
```{r echo=FALSE, cache=TRUE}
data(beetles_col_distM)
round(beetles_col_distM[1:3,1:3], 4)
```
...
data <- beetles_plotA
tree <- beetles_col_tree
distM <- beetles_col_distM

output_qiTD <- iNEXT.link(data, diversity = 'TD', q = c(0,1,2), nboot = 30)

output_qiPD  <- iNEXT.link(data, diversity = 'PD', q = c(0, 1, 2), nboot = 20, col.tree = tree)

output_qiFD  <- iNEXT.link(data, diversity = 'FD', q = c(0, 1, 2), nboot = 10, col.distM = distM, FDtype = 'AUC')
```
...
output_est_qiPD <- estimateD.link(beetles_plotA, diversity = 'PD', q = c(0,1,2), 
                                  base = "coverage", level = c(0.975, 0.99), col.tree = beetles_col_tree)
output_est_qiPD
```
...
output_est_qiFD <- estimateD.link(beetles_plotA, diversity = 'FD', q = c(0,1,2), nboot = 10,
                                  base = "size", level = c(500, 1000), col.distM = beetles_col_distM, FDtype = "AUC")
output_est_qiFD
```
...
output_ObsAsy_qiFD <- ObsAsy.link(data = beetles, diversity = 'FD', q = seq(0, 2, 0.25), 
                                  col.distM = beetles_col_distM, FDtype = "AUC", nboot = 10)
output_ObsAsy_qiFD
```
...
## <span style="color:red;">FUNCTION Spec.link and ggSpec.link: NETWORK SPECIALIZATION PROFILES</span>

`Spec.link()` the network specialization q-profiles for q between 0 and 2 in increments of 0.2 (by default) based on five classes of specialization measures, and function `ggSpec.link` is used to plot the corresponding specialization profiles. These two functions are specifically for users who only require specialization estimates and profiles. The two functions with arguments are described below: 

```{r eval=FALSE}
Spec.link(data, q = seq(0, 2, 0.2), method = "Estimated",
         nboot = 30, conf = 0.95, E.class = c(1:5), SC = NULL) 
```
...
A simple example for demonstrating functions `Spec.link` and `ggSpec.link` are given below. 
...
```{r, eval=FALSE}
data(beetles_plotA)
output_spec <- Spec.link(data = beetles_plotA)
output_spec[[1]]
```
