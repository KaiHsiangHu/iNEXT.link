...
author: "Anne Chao, Kai-Hsiang Hu"
date: "`r Sys.Date()`"
output:
...
**STEP 2. Sample-size- or size-based rarefaction and extrapolation analysis with 3D network diversity for q = 0, 1, and 2, and the asymptotic network diversity profile. Here size-based means the number of interaction events.**
...
tmp = output_beta_qiTD$Dataset$beta
...
tmp = output_beta_qiPD$Dataset$beta
tmp[,c(3:5, 7:9)] = round(tmp[,c(3:5, 7:9)], 3)
tmp = tmp[,-ncol(tmp)]
tmp
```
...
tmp = output_beta_qiFD$Dataset$beta
tmp[,c(3:5, 7:9)] = round(tmp[,c(3:5, 7:9)], 3)
tmp = tmp[,-ncol(tmp)]
tmp
```
