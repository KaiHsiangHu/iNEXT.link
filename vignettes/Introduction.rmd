...
<td align="center"><code>data</code></td>
<td align="left">
data should be input as a matrix/data.frame of species abundances (for a single assemblage), or a list of species abundance matrix/data(for multi-assemblage). </td>
...
The arguments of this function are briefly described below, and will be explained in more details by illustrative examples in later text. This main function computes the gamma, alpha, beta diversity, and four dissimilarity-turnover indices estimates base on sample coverage.
...
<td align="center"><code>data</code></td>
<td align="left">
data can be input as a <code>matrix<code>/<code>data.frame<code> (species by assemblages), or a <code>list<code> of <code>matrices<code>/<code>data.frames<code>, each <code>matrix<code> represents species-by-assemblages abundance <code>matrix<code></td>
